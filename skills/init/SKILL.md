...
**Log files are NOT copied here.** Logs (`TASK_LOG.md`, `DECISION_LOG.md`, etc.) are derived views generated by `ddw-index` — they will be gitignored and created on demand. Run `node ${CLAUDE_PLUGIN_DIR}/scripts/ddw-index.mjs` after init to generate them locally.
...
4. **Override** these fields with user-collected values:
   ...
   - `commands.test` ← user-selected test command, or `null`
...
## Step 6.5 — Patch `.gitignore`

Add DDW-managed entries to `.gitignore` in the project root. If `.gitignore` doesn't exist, create it.

Append the following block (or after any existing DDW section if one exists). Never reorder or remove existing lines:

```
# DDW — generated views and per-developer runtime state (do not commit)
{workflowDir}/logs/
{workflowDir}/.ddw/
```
